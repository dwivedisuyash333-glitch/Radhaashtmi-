(() => {
  "use strict";

  const canvas = document.getElementById("radhaCanvas");
  const ctx = canvas.getContext("2d", { alpha: true });
  const data = window.RADHA_PIXELS;

  if (!data || !data.pixels?.length) return;

  canvas.width = data.width;
  canvas.height = data.height;
  ctx.imageSmoothingEnabled = false;

  // Seeded shuffle = same beautiful reveal every time.
  let seed = 18092026;
  const rand = () => {
    seed = (seed * 1664525 + 1013904223) >>> 0;
    return seed / 4294967296;
  };

  const pixels = data.pixels.map(p => ({x:p[0],y:p[1],r:p[2],g:p[3],b:p[4],a:p[5]}));
  for (let i=pixels.length-1;i>0;i--) {
    const j=Math.floor(rand()*(i+1));
    [pixels[i],pixels[j]]=[pixels[j],pixels[i]];
  }

  ctx.clearRect(0,0,canvas.width,canvas.height);

  // Fast enough to feel magical, but still visibly pixel-by-pixel.
  const perFrame = Math.max(8, Math.floor(pixels.length / 180));
  let index=0;

  function reveal(){
    for(let i=0;i<perFrame && index<pixels.length;i++){
      const p=pixels[index++];
      ctx.fillStyle=`rgba(${p.r},${p.g},${p.b},${p.a/255})`;
      ctx.fillRect(p.x,p.y,1,1);
    }
    if(index<pixels.length) requestAnimationFrame(reveal);
    else finish();
  }

  const wait = ms => new Promise(resolve=>setTimeout(resolve,ms));

  async function finish(){
    await wait(900);
    document.getElementById("radheTitle").classList.add("visible");
    document.getElementById("hint").classList.add("show");

    // Each paragraph appears separately, exactly 5 seconds apart.
    await wait(1000);
    document.getElementById("greeting").classList.add("show");

    await wait(5000);
    document.getElementById("blessing").classList.add("show");

    await wait(5000);
    document.getElementById("anniversary").classList.add("show");

    await wait(700);
    petals();
  }

  function petals(){
    const box=document.getElementById("petals");
    const count=window.innerWidth<600?22:36;
    for(let i=0;i<count;i++){
      const p=document.createElement("span");
      p.className="petal";
      p.style.left=(Math.random()*100)+"%";
      p.style.setProperty("--drift",(-100+Math.random()*200)+"px");
      p.style.animationDuration=(6+Math.random()*5)+"s";
      p.style.animationDelay=(Math.random()*3)+"s";
      box.appendChild(p);
    }
  }

  setTimeout(reveal, 500);
})();
